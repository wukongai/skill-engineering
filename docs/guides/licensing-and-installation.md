...
| 让 Codex/Claude 发现 Skill Engineering | Agent Skill（标准 `skills` CLI） | 安装时选择 | `npx skills add wukongai/skill-engineering`；安装器交互选择宿主和 project/global 范围 |
...
安装器会交互选择 `skill-engineering`、Codex/Claude Code 等宿主和 project/global 范围。普通用户不需要记忆 `--skill`、`-g`、`-a` 或 `-y`。只有 CI、脚本化安装或明确知道目标范围时，才使用这些高级参数。升级和移除使用同一个 `skills` CLI；本项目不自动修改真实用户 Global 目录。
