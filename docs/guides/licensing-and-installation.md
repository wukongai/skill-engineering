...
标准标识为 `SPDX-License-Identifier: MIT`。完整条款以仓库根目录 [`LICENSE`](../../LICENSE) 为准。MIT 允许使用、复制、修改、发布、再许可和商业使用，但要求保留版权和许可证声明，并按“现状”提供，不承担保证或责任。

### 不自动纳入 MIT 的内容

- 第三方代码、数据、截图、商标、引用和外部项目内容：按其原许可证或来源要求处理；
- 用户提供的 prompt、私有数据、生成的 Skill、运行结果和业务材料：项目不主张其所有权；
- `Skill Engineering` 名称、Logo 和其他品牌资产：MIT 不等于商标授权。

当前仓库没有捆绑 NVIDIA/SkillSpector 的源代码；相关内容是外部项目比较和来源记录，不能当作本项目 MIT 代码分发。

贡献者提交代码或文档时，必须拥有相应权利，并同意该贡献按本项目 MIT 许可发布；项目不接收凭证、客户数据或完整私有对话。

许可证若要改变，必须单独建立 ADR、迁移说明、贡献者沟通和版本门禁；不会在补丁版本中静默改证。
...
可以承诺：MIT 开源核心、可自托管、允许商业使用和二次分发、CLI 与 Agent Skill 的安装边界透明、不会用封闭格式锁定用户。

暂时不要承诺：所有模型上的统一效果、托管协作已交付、企业 SLA、自动 Global 发布、用户生成 Skill 的独占版权或法律合规保证。
