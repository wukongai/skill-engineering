...
默认使用安装器的交互选择，让用户选择 Skill、目标宿主和 project/global 范围。`--skill`、`-g`、`-a` 和 `-y` 只用于用户已经明确知道目标，或 CI/脚本需要非交互执行的高级场景。
...
需要非交互自动化时，仍可显式增加 `--skill skill-engineering -g -a codex -y`；它是兼容的高级形式，不是普通用户默认入口。
