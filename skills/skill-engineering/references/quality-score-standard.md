...
新建 Skill 写入并通过结构 postflight 后,必须完成一次创建评审:先复跑内容完整性门禁,再按本文件的维度、权重和等级运行 Doctor v2 评分。不新建第二套评分模型。

- 评审结论先行:能否开始使用、分数和等级、最需要关注的一两个维度;规则明细折叠进“技术详情”。
- 存在 FAIL 或 SEC 命中时,创建状态不得高于 `candidate_incomplete`,并给出唯一修复入口。
- 没有真实任务试用前,必须同时说明“尚未验证实际任务效果”;分数只是结构和工程就绪度。
- 详细流程与用户可见状态机见 `references/creation-review.md`。
